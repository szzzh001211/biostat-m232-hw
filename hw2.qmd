...
Since $y_{i1}$ and $y_{i2}$ are jointly normal, the conditional distribution of $y_{i2}$ given $y_{i1}$ follows:
$$
y_{i2} | y_{i1} \sim \mathcal{N} \left( x_{i2} \beta_2 + \rho (y_{i1} - x_{i1} \beta_1), 1 - \rho^2 \right).
$$
The probability of missingness is then:
$$
\Pr(y_{i2} > 0 | y_{i1}) = 1 - \Phi \left( \frac{-x_{i2} \beta_2 - (\rho / \sigma_1)(y_{i1} - x_{i1} \beta_1)}{\sqrt{1 - \rho^2}} \right).
$$
This confirms the given expression. 

Next, we determine whether the missing data mechanism is **Missing at Random (MAR)** and whether it is **ignorable** for likelihood-based inference. The missingness probability depends on $y_{i1}$ (an observed variable) and $x_{i2}$ (also observed), but not directly on $y_{i2}$ (which is missing). Therefore, the missing data mechanism is **MAR**. However, since the missingness probability explicitly involves parameters $\beta_1, \beta_2$, and $\rho$, the missing data mechanism is **not ignorable** for likelihood inference, meaning that the likelihood function must account for the missingness model explicitly.

Thus, the missing data mechanism in this problem is **MAR but not ignorable**, necessitating explicit modeling in likelihood-based inference.
...
$$
$$
\frac{d \log L_c}{d \sigma^2} = \frac{1}{2\sigma^4} \sum_{i=1}^{n} Z_i (y_i - \mu_1)^2 + \frac{1}{2\sigma^4} \sum_{i=1}^{n} (1 - Z_i) (y_i - \mu_0)^2 - \frac{n}{\sigma^2} = 0
$$
So the MLE of $\sigma^2$ is:
$$
\hat{\sigma}^2 = \frac{\sum_{i=1}^{n} Z_i (y_i - \hat{\mu}_1)^2 + \sum_{i=1}^{n} (1 - Z_i) (y_i - \hat{\mu}_0)^2}{2n}
$$
