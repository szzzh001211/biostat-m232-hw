...
$$
\text{Var}(\hat{\Delta}_{AC}) = \frac{\sigma^2}{n} + \frac{\sigma^2}{m}
$$

For the variance of AC to be less than that of CC, we need:
$$
\begin{align*}
&\frac{\sigma^2}{n} + \frac{\sigma^2}{m} < \frac{2\sigma^2(1 - \rho)}{m}\\
&\Rightarrow \frac{1}{n} + \frac{1}{m} < \frac{2(1 - \rho)}{m}\\
&\Rightarrow \frac{1}{n} < \frac{2(1 - \rho)}{m} - \frac{1}{m}\\
&\Rightarrow \frac{1}{n} < \frac{1 - 2\rho}{m}\\
&\Rightarrow n > \frac{m}{1 - 2\rho} \quad (\rho < 0.5)
\end{align*}
$$
